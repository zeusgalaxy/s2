...
case class Exerciser(dbId: Long, login: String, email: String, pic: Int,
                     ...
                     gigyaUid: String, showProfilePic: Boolean, webPassword: Option[String]) {

  def hasVtConnection = (!vtUserId.isEmpty && !vtToken.isEmpty && !vtTokenSecret.isEmpty)

  def genderStr = gender ? 'F' | 'M'
...
trait ExerciserDao {
  ...
  lazy val exSelectFields = " exerciser.id, exerciser.login, exerciser.email, exerciser.pic, " +
    ...
    " exerciser.gigya_uid, exerciser.show_profile_pic, exerciser.web_password "
  ...
  lazy val exSimple = {
    get[Long]("exerciser.id") ~
      ...
      get[Boolean]("exerciser.show_profile_pic") ~
      get[Option[String]]("exerciser.web_password") map {
  ...
  def exFindByDbId(dbId: Long): Option[Exerciser] = {
    ...
    implicit val loc = VL("ExerciserDao.findById")
  ...
  def exFindByLogin(login: String): Option[Exerciser] = {
    ...
    implicit val loc = VL("ExerciserDao.findByLogin")
  ...
  def exGetId(login: String): Option[Int] = {
    ...
    implicit val loc = VL("ExerciserDao.getId")
  ...
  def exGetSavedChannels(login: String, locationId: Long): List[Long] = {
    ...
    implicit val loc = VL("ExerciserDao.getSavedChannels")
  ...
  def exSetSavedChannels(login: String, locationId: Long, channels: List[Long]): Boolean = {
    ...
    implicit val loc = VL("ExerciserDao.setSavedChannels")
  ...
  def exLoginVt(npLogin: String, vtUserId: String, vtToken: String, vtTokenSecret: String): Boolean = {
    ...
    implicit val loc = VL("ExerciserDao.loginVt")
  ...
  def exLogoutVt(npLogin: String): Boolean = {
    ...
    implicit val loc = VL("ExerciserDao.logoutVt")
  ...
  def exSetGigyaUid(npLogin: String, gigyaUid: String): Boolean = {
    ...
    implicit val loc = VL("ExerciserDao.setGigyaUid")
  ...
  /**Adds this newly registered exerciser to the s2 database. Previously, these were
   * transferred from titan to s2 in an ETL process, but the lag time was causing problems.
   * Now we do the update directly into s2 (immediately following the titan registration).
   *
   * @param npLogin The exerciser's login identifier
   * @return true if successful, else false
   */
      ...
      DB.withConnection("s2") {
        implicit connection =>
          SQL(
            """
              insert into person
              set role_id = (select id from role where name = "exerciser" limit 1),
              first_name = '', last_name = '', portal_login = {portalLogin}, portal_password = {portalPassword},
              email = {email}, phone = '', created_at = now(), created_by = 0
            """
          ).on(
            'portalLogin -> ex.email.trim,
            'portalPassword -> ex.webPassword.getOrElse(null),
            'email -> ex.email
          ).executeUpdate()

          val pid = SQL("select last_insert_id() as id").as(scalar[Long].single)
          ...
          SQL(
            """
              insert into titan_id_map
              set titan_id = {exId}, titan_object = 'exerciser', s2_object = 'person',
              s2_id = {s2Id}, created_by = 0, created_at = now()
            """
          ).on(
            'exId -> ex.dbId,
            's2Id -> pid
